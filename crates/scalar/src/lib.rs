...
use consensus_client::GrpcConsensusClient;
...
pub use handle::ScalarNodeHandle;
...
use sui_core::consensus_validator::SuiTxValidatorMetrics;
...
use crate::metrics::{GrpcMetrics, ScalarNodeMetrics};

pub mod admin;
pub(crate) mod consensus_client;
pub(crate) mod consensus_manager;
mod handle;
pub mod metrics;
pub(crate) mod proto;
...
impl ScalarNode {
    ...
    pub async fn start_async(
        ...
        let sui_node_metrics =
            Arc::new(ScalarNodeMetrics::new(&registry_service.default_registry()));
    ...
    async fn construct_validator_components(
        ...
        connection_monitor_status: Arc<ConnectionMonitorStatus>,
        registry_service: &RegistryService,
        sui_node_metrics: Arc<ScalarNodeMetrics>,
        ...
        let client = Arc::new(GrpcConsensusClient::new(epoch_store.get_chain_identifier()));
        ...
        let consensus_manager = ConsensusManager::new(registry_service, client);
    ...
    async fn start_epoch_specific_validator_components(
        ...
        consensus_manager
            .start(config, consensus_handler_initializer)
            .await;
    ...
    pub async fn monitor_reconfiguration(self: Arc<Self>) -> Result<()> {
        ...
        loop {
            ...
            if stop_condition == StopReason::RunWithRangeCondition {
                ScalarNode::shutdown(&self).await;
