...
fn main() {
    ...
    // if it deadlocks.
    let node_once_cell = Arc::new(AsyncOnceCell::<Arc<scalar::ScalarNode>>::new());
    ...
    runtimes.sui_node.spawn(async move {
        match scalar::ScalarNode::start_async(config, registry_service, Some(rpc_runtime), VERSION).await {
            Ok(scalar) => node_once_cell_clone
                .set(scalar)
    ...
    runtimes.metrics.spawn(async move {
        ...
        scalar::admin::run_admin_server(node, admin_interface_port, filter_handle).await
    });
