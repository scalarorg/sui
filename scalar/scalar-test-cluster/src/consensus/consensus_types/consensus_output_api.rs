...
use crate::consensus::consensus_types::ConsensusTransactionWrapper;

use super::AuthorityIndex;
    ...
    fn transactions(&self) -> ConsensusOutputTransactions {
        self.sub_dag
            ...
            .map(|(cert, batches)| {
                ...
                let transactions: Vec<(&[u8], ConsensusTransactionWrapper)> = cert
                    .header()
                    .system_messages()
                    .iter()
                    .filter_map(|msg| {
                        ...
                        if let SystemMessage::RandomnessSignature(round, bytes) = msg {
                            Some((
                                [0u8; 0].as_slice(),
                                ConsensusTransactionWrapper::Consensus(ConsensusTransaction {
                                    tracking_id: [0; 8],
                                    kind: ConsensusTransactionKind::RandomnessStateUpdate(
                                        round.0,
                                        bytes.clone(),
                                    ),
                                }),
                    ...
                    .chain(batches.iter().flat_map(|batch| {
                        let digest = batch.digest();
                        assert!(cert.header().payload().contains_key(&digest));
                        batch
                            .transactions()
                            .iter()
                            .map(move |serialized_transaction| {
                                let transaction = bcs::from_bytes::<ConsensusTransaction>(
                                    serialized_transaction,
                                )
                                .map_or_else(
                                    |err| match bcs::from_bytes::<ConsensusTransactionWrapper>(
                                        serialized_transaction,
                                    ) {
                                ...
                                //     serialized_transaction,
                                // ) {
                                //     Ok(transaction) => ConsensusTransactionWrapper::Consensus(transaction),
                                //     Err(err) => {
                                //         // This should have been prevented by Narwhal batch verification.
                                //         panic!(
                                //             "Unexpected malformed transaction (failed to deserialize): {}\nCertificate={:?} BatchDigest={:?} Transaction={:?}",
        ...
        self.blocks
            ...
            .map(|block| {
                ...
                let transactions: Vec<_> = block
                    .shared_transactions()
                    .flat_map(|(_loc, tx)| {
                        let transaction = bcs::from_bytes::<ConsensusTransactionWrapper>(tx.data());
                        match transaction {
                            Ok(transaction) => Some((
                                tx.data(),
                                transaction,
